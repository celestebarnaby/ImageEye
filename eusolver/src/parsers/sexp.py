...
class Img(object):
    ...
    def __eq__(self, other):
      return self.attrs == other.attrs and self.left == other.left and self.right == other.right and self.top == other.top and self.bottom == other.bottom

    def __str__(self):
      return "Img(" + str(self.attrs) + ", BB HERE" + ")"

    def __hash__(self):
      return hash((self.attrs, self.left, self.right, self.top, self.bottom))
...
def handleImg(s):
  if s[0] == '{' and s[-1] == '}':
    s = s[1:-1]
  l = s.split("bb:")
  attrs = [attr.strip() for attr in l[0].split(' ') if attr]
  bounds = [int(bound) for bound in l[1].split(' ') if bound]
  # return Img(attrs, bounds)
  return (frozenset(attrs), bounds[0], bounds[1], bounds[2], bounds[3], bounds[4])
...
set_ = Regex(r"{[\{\}:\d\w,\s-]*}").setParseAction(lambda t: ('Imgs', handleSet(t[0])))
# set_ = Regex(r"{[^\)]+}").setParseAction(lambda t: ('Imgs', handleSet(t[0])))
...
simpleString = real | base64_ | raw | decimal | token | hexadecimal | qString | set_ 
