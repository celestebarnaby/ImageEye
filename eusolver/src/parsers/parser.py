...
from semantics import semantics_img
from core import synthesis_context
from exprs import exprtypes
from eusolver_utils.bitvectors import BitVector
...
def sexp_to_value(sexp):
    ...
    if value_type.type_code == exprtypes.TypeCodes.integer_type:
        ...
        value = value_exp
    elif value_type.type_code == exprtypes.TypeCodes.set_type:
        value = value_exp
    elif value_type.type_code == exprtypes.TypeCodes.imgs_type:
        value = value_exp
    else:
...
def sexp_to_type(sexp):
    if type(sexp) == list and sexp[0] == 'BitVec':
        ...
        return exprtypes.SetType()
    elif sexp == 'Imgs':
        return exprtypes.ImgsType()
    else:
...
_known_theories = [ "LIA", "BV", "SLIA", "IMG"]
...
def get_theory_instantiator(theory):
    if theory == "LIA":
        ...
        return semantics_img.IMGInstantiator()
    else:
